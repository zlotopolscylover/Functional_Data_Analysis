...
X10<-sapply(1:50, function(x) simGauss(autocov)+4*t) #101x10
fd10<-fdata(t(X10))

fd10$names$main <- "Proces Gaussowski 50 próbek ze œredni¹ okreœlon¹ w pkt a)"
fd10$names$xlab = "t"
fd10$names$ylab = "Y(t)"
fd10$argvals = X_argval
fd10$rangeval = c(min(X_argval),max(X_argval))
m1<-func.mean(fd10)
plot(fd10,cex.lab=1.5,col="Grey")
lines(m1,col="red",lwd=2)

#Graficzna reprezentacja danych dla 10 probek:
X10<-sapply(1:50, function(x) simGauss(autocov)+4*t+2) #101x10
fd10<-fdata(t(X10))

fd10$names$main <- "Proces Gaussowski 50 próbek ze œredni¹ okreœlon¹ w punkcie b)"
fd10$names$xlab = "t"
fd10$names$ylab = "Y(t)"
fd10$argvals = X_argval
fd10$rangeval = c(min(X_argval),max(X_argval))
...
fd10$names$main <- "Proces Gaussowski dla 50 próbek ze œredni¹ okreœlon¹ w pkt c)"
fd10$names$xlab = "t"
fd10$names$ylab = "Y(t)"
fd10$argvals = X_argval
fd10$rangeval = c(min(X_argval),max(X_argval))
...
fd10$rangeval = c(min(X_argval),max(X_argval))
m<-func.mean(fd10)
plot(m1,cex.lab=2,col="red",title="Œrednie")
lines(m2,col="blue",lwd=2)
lines(m3,col="green",lwd=2)
...
X<-sapply(1:50, function(x) simGauss(autocov)+4*t)
fd<-fdata(t(X))
fd$argvals = X_argval
fd$rangeval = c(min(X_argval),max(X_argval))
m<-func.mean(fd)
